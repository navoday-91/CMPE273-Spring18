...
socketsub.connect ("tcp://localhost:%s" % port)
port = "5999"
context = zmq.Context()
socketpub = context.socket(zmq.PUB)
socketpub.connect("tcp://localhost:%s" % port)
publisher_id = 5
topicfilter = '9'
...
class MainWindow(object):
    ...
    signals = ["quit", "keypress", "print_received_message"]
    ...
    def print_sent_message(self, text):
        ...
        socketpub.send_string("%d %s" % (9, name + ":" + text))
    ...
    def print_received_message(self):
        ...
        while True:
            ...
            string = socketsub.recv().decode()
            topic, messagedata = string.split(" ",1)
            from_client, messagedata = messagedata.split(":",1)
            if messagedata != "":
                if name!=from_client:
                    header = urwid.Text('['+self.get_time()+'] '+ from_client + ':')
                    header.set_align_mode('right')
                    self.print_text(header)
                    text = urwid.Text(messagedata)
...
if __name__ == "__main__":
    ...
    receiver = threading.Thread(target=main_window.print_received_message)
